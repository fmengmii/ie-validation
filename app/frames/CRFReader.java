...
public class CRFReader
{
	...
	private String getHTMLString(String elementType, String htmlID, List<Map<String, String>> values, int repeat, int elementRepeatNum, String repeatStr)
	{
		...
		if (elementType.equals("radio") || elementType.equals("checkbox")) {
			...
			htmlStr = "<select id='" + htmlID + "' onclick='stopEvent(event);'>";
			StringBuilder strBlder = new StringBuilder();
			...
			//htmlStr += "<script>$('#" + htmlID + "').bind('click', function(){alert('hi!');});</script>";
	...
	private int getSectionID(int crf_id, String section) throws SQLException
	{
		Statement stmt = conn.createStatement();
		ResultSet rs = stmt.executeQuery("select section_id from " + schema + "crf_section where name = '" + section + "' and crf_id = " + crf_id);
		int sectionID = -1;
		if (rs.next()) {
	...
	public static void main(String[] args)
	{
		...
		try {
			CRFReader reader = new CRFReader(args[5] + ".");
			//reader.readCRFFile(args[0], args[1], args[2], args[3], args[4], args[6]);
		}
		catch(Exception e)
		{
			e.printStackTrace();
